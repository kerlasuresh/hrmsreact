...
const Directory = () => {
  ...
  const [resendActivationSuccessPopup, setresendActivationSuccessPopup] = useState(false)
  const [pleaseSelectOneEmployee, setpleaseSelectOneEmployee] = useState(false)
  const [importEmployeeSuccessPopup, setimportEmployeeSuccessPopup] = useState(false)
  const [errorImportEmployees, seterrorImportEmployees] = useState(false)
  const [exportSuccessfullMesage, setexportSuccessfullMesage] = useState(false)
  ...
  const [selectsub_departmenterror, setselectsub_departmenterror] = useState('')
  ...
  const [importemployeeMessageError, setimportemployeeMessageError] = useState("")
  ...
  const individualCheckbox = (value, email, phone, emp, empID) => {
    ...
    setresendActivations([...resendActivations, { "email": email, "phone": phone }])
  ...
  const columns = [
    {
      ...
      selector: (row, index) => (
        <input
          type="checkbox"
          data-id={index + 1}
          onChange={(e) => individualCheckbox(e.target.checked, row.officialEmail, row.phone, row, row.id)}
    ...
    grade
      ? {
          name: <span className="h6 fw-bold">Grade</span>,
        selector: (row) => row.employeeGrade,
          sortable: true,
        }
      : {
          omit: hideDirector,
    ...
    email
      ? {
          name: <span className="h6 fw-bold">Email</span>,
    ...
    phone
      ? {
          name: <span className="h6 fw-bold">Phone</span>,
    ...
    DOB
      ? {
          name: <span className="h6 fw-bold">Date of Birth</span>,
    ...
    signedIn
      ? {
          name: <span className="h6 fw-bold">Signed In</span>,
          selector: (row) =>(row.signedIn === true ? 'YES' : <div>
            <button className='btn btn-primary btn-sm' >Re-Invite</button>
          </div>),
          sortable: true,
        }
      : {
    ...
    employeeType
      ? {
          name: <span className="h6 fw-bold">Employee Type</span>,
    ...
    attendanceRules
      ? {
          name: <span className="h6 fw-bold">Attendance Rules</span>,
          selector: (row) => '-',
          sortable: true,
        }
      : {
    ...
    showlist
      ? {
          name: <span className="h6 fw-bold">Workweek Rules</span>,
          selector: (row) => '-',
          sortable: true,
        }
      : {
  ...
  const getAttendanceRules = (value) => {
    setAttendanceRules(value)
    setHideDirector(!hideDirector)
  }
  const getLeaveRules = (value) => {
    setLeaveRules(value)
    setHideDirector(!hideDirector)
  }

  // const bulkeditShowpopup = () => {
 ...
 const bulkEditInnerPopUpClose = () => {
  ...
  const bulkEditInnerSaveSection = async () => {
    ...
    if (selectedIDEmp !== 0){
      ...
      var req = {
        ...
        "effectiveDate": getValues('selectedEffectiveDate') ? moment(getValues('selectedEffectiveDate')).format("DD-MM-YYYY") : ""
      }
      console.log(req)
      if (getValues('selectedEffectiveDate')){
        if (getValues('selectDepartment') || getValues('selectreportingManager') || getValues('selectwork_location') || getValues('selectdesignation') || getValues('selectEmployeeGrade') || getValues('selectemployeeStatus') || getValues('selectemployee_type')){

        
      
      try {
        const response = await axios.post(`/api/directory/bulk_update/employee/`, req)
        if (response.data) {
  ...
  const resendActivationLink = async () => {
    if (resendActivations.length !== 0){
      try {
        const response = await axios.post(`/api/user/resend-activation-link/`, resendActivations)
        if (response.data){
  ...
  const importEmployeeFiles = async () => {
    ...
    if (FilesList[0]){
      ...
      try {
        const response = await axios.post(`/api/directory/import/employee/`, formData)
        if (response.data) {
  ...
  const exportToFileManagerFolder = async () => {
    var req = {
      "id": selectedIDEmp,
      "fields": []
    }
    try {
      const response = await axios.post(`/api/reports/genearate/employee/`, req)
      if (response.data) {
        setselectedEmp([])
        setmailsLists([])
        setselectedIDEmp([])
        setresendActivations([])
        setselectedEmployees(0)
        setexportSuccessfullMesage(true)
        setExportHoleData(false);
        $('.individualcheckbox').prop('checked', false)
        $('.selectAll').prop('checked', false)
      }
    } catch (error) {
    ...
    <>
            ...
            <div className="col-md-12 mt-4">
              ...
              <div className="tableexportInfo">
                <div className="optioninfo">
                  ...
                  <div
                    className="mailSendEmployee"
                    mailto="sreekanth@vitelglobal.com"
                  >
                    <div onClick={()=>sendMailforAll()}>
                      <div className="bulkedit">
                        <span>
                          <AiTwotoneMail />
                        </span>
                      </div>
                      <div className="textInfo">Mail</div>
                    </div>
                  </div>
                  <div
      ...
      >
        ...
        <Modal.Body>
          <div className="row">
            <div className="col-md-6">
              <div className="select">{selectedEmployees} Employee Selected</div>
              <br />
            </div>
            <div className="col-md-12">
              <div className="select">Selected Fields</div>
              <div className="selectedFields">
                <ul>
                  ...
                  <li>Department</li>
                  <li>Designation</li>
                  <li>Primary Manager</li>
                  <li>Secondary Managers</li>
                  <li>Job Title</li>
                  <li>DOJ</li>
                  <li>Work Location</li>
                  ...
                  <li>Employee Emergency Contacts</li>
                  <li>CTC</li>
                </ul>
              </div>
            </div>
            ...
            <div className="col-md-12">
              {/*<div className="select">Select Optional Fields</div>
               <div className="selectedFields">
                <ul>
                  <li>
                    <input type="checkbox" /> Sub Department
                  </li>
                  <li>
                    <input type="checkbox" /> Reportees
                  </li>
                  <li>
                    <input type="checkbox" /> Marital Status
                  </li>
                  <li>
                    <input type="checkbox" /> Marriage Anniversary
                  </li>
                  <li>
                    <input type="checkbox" /> Blood Group
                  </li>
                  <li>
                    <input type="checkbox" /> Social Media Links
                  </li>
                  <li>
                    <input type="checkbox" /> Current Address
                  </li>
                  <li>
                    <input type="checkbox" /> Permanent Address
                  </li>
                  <li>
                    <input type="checkbox" /> Educational Info
                  </li>
                  <li>
                  ...
                  <li>
                    <input type="checkbox" /> Other Documents
                  </li>
                  <li>
                    <input type="checkbox" /> Termination Date
                  </li>
                  <li>
                    <input type="checkbox" /> Work History
                  </li>
                  <li>
                    <input type="checkbox" /> Leave Rules
                  </li>
                  <li>
                    <input type="checkbox" /> Attendance Rules
                  </li>
                  <li>
                    <input type="checkbox" /> Workweek Rules
                  </li>
                  <li>
          ...
          <Button variant="danger" onClick={() => setExportHoleData(false)}>
            Cancel
          </Button>{' '}
          &nbsp;
          <Button variant="primary" onClick={()=>exportToFileManagerFolder()}>Export</Button>
          </div>
        </Modal.Body>
      </Modal>
      ...
      >
        ...
        <Modal.Body>
          <div className="row">
            <div className="col-md-12">
              <div className="SelectFileType">
                <input type="file" {...register('employee_file', {
              ...
              <div className="">
                {/* <input type="radio" htmlFor="quickAdd" name="import" /> */}
                <a href="http://38.143.106.215:82/media/download/bulk_upload_template_pranathi.xlsx" rel="noreferrer" target="_blank">Download Sample import file</a>
                {/* <br />
                <input type="radio" id="bulkAdd" name="import" />
                <span htmlFor="bulkAdd">Bulk Add - Advanced</span>
                <br />
                <input type="radio" id="updateID" name="import" />
                <span htmlFor="updateID">Update Employee ID</span>
                <br /> */}
              </div>
            </div>
          </div>
          ...
          &nbsp;
          <Button variant="primary" onClick={()=>importEmployeeFiles()}>Import File</Button>
          </div>
        </Modal.Body>
      </Modal>
      ...
      >
        ...
        <Modal.Body>
          ...
          <br />
          <br />
          <Button variant="primary" onClick={()=>resendActivationLink()}>Yes</Button> &nbsp;
          <Button variant="danger" onClick={() => setresendActivation(false)}>
            No
          </Button>
        </Modal.Body>
      </Modal>
      ...
      >
        <Modal.Header closeButton>
          {' '}
          <h4 className="text-start">Bulk Edit </h4>
        </Modal.Header>
        { selectedEmployees > 0 ?  <div className="text-start ml-2">
          <div className="mb-2" onClick={() => getBulkEditValue('Department')}>
            Department
          </div>
          <div className="mb-2" onClick={() => getBulkEditValue('Designation')}>
            Designation{' '}
          </div>
          <div
            className="mb-2"
          ...
          <div className="mb-2" onClick={() => getBulkEditValue('Grades')}>
            Grades
          </div>
          <div className="mb-2" onClick={() => getBulkEditValue('Location')}>
            Location
          </div>
          <div
            className="mb-2"
          ...
          >
            ...
            <div className='text-center'>
              <AiOutlineInfoCircle style={{ color: "#3fc3ee", margin: "30px" }} size={100} />
            </div>
            <h5>  please select atleast one employee </h5>
            <div className='text-center m-3'>
              <button className='btn btn-primary' onClick={() => { setBulkEditShowPopUp(false); }}> OK</button>
            </div>
           </div>}
      </Modal>
        ...
        <div className="row m-3">
          ...
          <div className="col-md-6">
            <div class="input-group mb-3">
              ...
              <input type="date" class="form-control" {...register('selectedEffectiveDate', {
          ...
          <div>
            ...
            <div className="row m-3">
              ...
              <div className="col-md-6">
                ...
                <small className='text-danger'>{selectsub_departmenterror}</small>
          ...
          <div>
            ...
            <div className=" w-50 m-auto">
              ...
              >
                ...
                {Gradess.map((grade,i)=> {
                  return (
                    <option value={grade.id} key={i}>{grade.grade}</option>
                  )
                })}
      ...
      <Modal size='md' show={bulkeditSuccessPopup} onHide={() => setbulkeditSuccessPopup(false)} className='text-center'>
        <Modal.Header closeButton>Success</Modal.Header>
        <h4 className='text-center'>Updated Successfully</h4>
        <div className='text-center m-3'>
          <button className='btn btn-primary' onClick={() => { setbulkeditSuccessPopup(false); }}> OK</button>
        </div>
      </Modal>
      <Modal size='md' show={importEmployeeSuccessPopup} onHide={() => setimportEmployeeSuccessPopup(false)} className='text-center'>
        <Modal.Header closeButton>Success</Modal.Header>
        <h4 className='text-center'>Employee Details imported Successfully</h4>
        <div className='text-center m-3'>
          <button className='btn btn-primary' onClick={() => { setimportEmployeeSuccessPopup(false); }}> OK</button>
        </div>
      </Modal>
      <Modal size='md' show={resendActivationSuccessPopup} onHide={() => setresendActivationSuccessPopup(false)} className='text-center'>
        <Modal.Header closeButton>Success</Modal.Header>
        <h4 className='text-center'>Resend Activation Successfully</h4>
        <div className='text-center m-3'>
          <button className='btn btn-primary' onClick={() => { setresendActivationSuccessPopup(false); }}> OK</button>
        </div>
      </Modal>

      <Modal size='md' show={pleaseSelectOneEmployee} onHide={() => setpleaseSelectOneEmployee(false)} className='text-center'>
        ...
        <div className='text-center m-3'>
          <button className='btn btn-primary' onClick={() => { setpleaseSelectOneEmployee(false); }}> OK</button>
        </div>
      </Modal>

      <Modal size='md' show={errorImportEmployees} onHide={() => seterrorImportEmployees(false)} className='text-center'>
        <Modal.Header closeButton>Alert</Modal.Header>
        
        <h4 className='text-center'>{importemployeeMessageError}</h4>
        <div className='text-center m-3'>
          <button className='btn btn-primary' onClick={() => { seterrorImportEmployees(false); }}> OK</button>
        </div>
      </Modal>

      <Modal size='md' show={exportSuccessfullMesage} onHide={() => setexportSuccessfullMesage(false)} className='text-center'>
        <Modal.Header closeButton>Alert</Modal.Header>
        <div className='text-center m-3'>
          <FcOk size='80px' />
        </div>
        <h4 className='text-center'>Exported Data successfully</h4>
        <div className='text-center m-3'>
          <button className='btn btn-primary' onClick={() => { setexportSuccessfullMesage(false); }}> OK</button>
        </div>
      </Modal>
    </>
  )
